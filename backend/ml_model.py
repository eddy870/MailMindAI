...
class EmailCampaignMLModel:
    """
    Machine Learning model for predicting email campaign performance improvements.
    Uses historical campaign data to predict improvement percentages for new campaigns.
    """
    
    def __init__(self):
        self.model_open_rate = RandomForestRegressor(n_estimators=100, random_state=42)
        self.model_click_rate = RandomForestRegressor(n_estimators=100, random_state=42)
        self.model_conversion_rate = RandomForestRegressor(n_estimators=100, random_state=42)
        
        # Encoders for categorical features
        self.target_market_encoder = LabelEncoder()
        self.target_industry_encoder = LabelEncoder()
        self.target_company_size_encoder = LabelEncoder()
    ...
    def prepare_features(self, campaigns_data: List[Dict]) -> pd.DataFrame:
        """
        Prepare feature matrix from campaign data.
        Includes both numerical and encoded categorical features.
        """
        df = pd.DataFrame(campaigns_data)
        
        # Fill missing categorical values
        df['target_market'] = df['target_market'].fillna('Unknown')
        df['target_industry'] = df['target_industry'].fillna('Unknown')
